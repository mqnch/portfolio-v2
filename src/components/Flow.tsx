...
const Flow: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      // Lower alpha (0.12 instead of 0.3) makes trails significantly longer and more visible
      ctx.fillStyle = "rgba(10, 10, 10, 0.12)";
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      particles.forEach(p => {
        p.update(canvas.width, canvas.height);
        // Keep the brightness for panel compatibility, but use transparency to soften them
